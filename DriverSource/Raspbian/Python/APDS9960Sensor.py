...
# Broadcom / Avago APDS-9960 Ambient Light, Color, Proximity & Gesture Sensor Python Driver
...
def Read8BytesArray(regAddr):
    """Reads data from the I2C device.
    :param regAddr: Read register address.
    """
    return bus.read_i2c_block_data(I2C_ADDRESS, regAddr, 8)
...
def SetInitialSettings():
    ...
    WriteRegister(APDS9960_ENABLE, 0x01)

    #Set ATIME to 72 cycles, 200ms, 65535 max count
    WriteRegister(APDS9960_ATIME, 0xB6)

    #Set wait time to 20ms
    WriteRegister(APDS9960_WTIME, 0xF9)

    #Set ALS interrupt low thresold
    WriteRegister(APDS9960_AILTL, 0xFF) #Low
    WriteRegister(APDS9960_AILTH, 0xFF) #High

    #Set ALS interrupt high thresold
    WriteRegister(APDS9960_AIHTL, 0x00) #Low
    WriteRegister(APDS9960_AIHTH, 0x00) #High

    #Set proximity interrupt low thresold
    WriteRegister(APDS9960_PILT, 0)

    #Set proximity interrupt high thresold
    WriteRegister(APDS9960_PIHT, 255)

    #Set interrupt persistence filters
    WriteRegister(APDS9960_PERS, 0x11)

    #Set configuration register one: No 12x wait (WLONG = 0)
    WriteRegister(APDS9960_CONFIG1, 0x60)

    #Set proximity pulse count to 8 and lenght to 8us
    WriteRegister(APDS9960_PPULSE, 0x48)

    #Set control register one:
    #LDRIVE: LED Drive Strenght to 100mA
    #PGAIN: Proximity Gain to 4x
    #AGAIN: ALS and Color Gain to 4x
    WriteRegister(APDS9960_CONTROL, CONTROL_LDRIVE.MA_100 | CONTROL_PGAIN.X4 | CONTROL_AGAIN.X04)
    ...
    WriteRegister(APDS9960_GCONF2, GCONF2_GGAIN.X4 | GCONF2_GLDRIVE.MA_100 | GCONF2_GWTIME.MS_02_8)

    #Set gesture offsets to 0
    WriteRegister(APDS9960_GOFFSET_U, 0x00)
    WriteRegister(APDS9960_GOFFSET_D, 0x00)
    WriteRegister(APDS9960_GOFFSET_L, 0x00)
    WriteRegister(APDS9960_GOFFSET_R, 0x00)

    #Set gesture pulse count to 32 and lenght to 8us
    WriteRegister(APDS9960_GPULSE, 0x96)
    ...
    WriteRegister(APDS9960_GCONF4, GCONF4_GFIFO_CLR.OFF | GCONF4_GIEN.OFF | GCONF4_GMODE.ALS_PROX_COLOR)

def SetMode(ambientAndRGBLightEnabled, proximityDetectionEnabled, gestureRecognitionEnabled):
    """Toggles the sensor modes.
    :param ambientAndRGBLightEnabled: Ambient light and RGB light sense.
    :param proximityDetectionEnabled: Proximity detection.
    :param gestureRecognitionEnabled: Gesture recognition.
    """
    enableCommand = 0x00
    ...
    WriteRegister(APDS9960_ENABLE, enableCommand)

#Sensor Readouts

def ReadAmbientLight():
    """Reads the ambient light value."""
    return int(Read2BytesAsUShortLSBFirst(APDS9960_CDATAL))
...
def ReadRGBLight():
    ...
    crgb = Read8BytesArray(APDS9960_CDATAL)
...
def ReadProximity():
    ...
    return int(ReadRegisterOneUByte(APDS9960_PDATA))
