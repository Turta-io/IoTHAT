...
# Bosch Sensortec BME680 Environmental Sensor Python Driver
...
def CheckSensor():
    ...
    return True if (ReadRegisterOneUByte(BME680_ID) == 0x61) else False

def ResetSensor():
    """Initiates a soft-reset procedure, which has the same effect like power-on reset."""
    WriteRegister(BME680_RESET, 0xB6)

def ConfigureSensor(temperatureOversampling, pressureOversampling, humidityOversampling, iirFilter, heatDuration, heatTemperature):
    ...
    :param iirFilter: IIR Filter.
    :param heatDuration: Gas sensor heat duration in ms. Max value is 252.
    :param heatTemperature: Gas sensor heat temperature in C. Max value is 400.
    """
    #Select humidity oversampling.
    WriteRegister(BME680_CTRL_HUM, humidityOversampling)
    time.sleep(0.001)

    #Select temperature and pressure oversamplings.
    configValue = 0x00
    configValue |= temperatureOversampling
    configValue |= pressureOversampling
    WriteRegister(BME680_CTRL_MEAS, configValue)
    time.sleep(0.001)

    #Select IIR Filter for temperature sensor.
    WriteRegister(BME680_CONFIG, iirFilter)
    time.sleep(0.001)
    ...
    WriteRegister(BME680_GAS_WAIT_0, CalculateHeatDuration(heatDuration))
    time.sleep(0.001)

    #Set heater temperature.
    WriteRegister(BME680_RES_HEAT_0, CalculateHeaterResistance(heatTemperature))
    time.sleep(0.001)

    #Set mode to forced mode.
    configValue = ReadRegisterOneUByte(BME680_CTRL_MEAS)
...
def ReadCalibrationData():
    ...
    calT1 = Read2BytesAsUShortLSBFirst(BME680_T1_LSB_REG)
    calT2 = Read2BytesAsShortLSBFirst(BME680_T2_LSB_REG)
    calT3 = ReadRegisterOneSByte(BME680_T3_REG)

    #Pressure calibration.
    calP1 = Read2BytesAsUShortLSBFirst(BME680_P1_LSB_REG)
    calP2 = Read2BytesAsShortLSBFirst(BME680_P2_LSB_REG)
    calP3 = ReadRegisterOneSByte(BME680_P3_REG)
    calP4 = Read2BytesAsShortLSBFirst(BME680_P4_LSB_REG)
    calP5 = Read2BytesAsShortLSBFirst(BME680_P5_LSB_REG)
    calP6 = ReadRegisterOneSByte(BME680_P6_REG)
    calP7 = ReadRegisterOneSByte(BME680_P7_REG)
    calP8 = Read2BytesAsShortLSBFirst(BME680_P8_LSB_REG)
    calP9 = Read2BytesAsShortLSBFirst(BME680_P9_LSB_REG)
    calP10 = ReadRegisterOneUByte(BME680_P10_REG)

    #Humidity calibration.
    calH1 = ReadRegisterOneUByte(BME680_H1_MSB_REG) << 4 | (ReadRegisterOneUByte(BME680_H1_LSB_REG) & 0x0F)
    calH2 = ReadRegisterOneUByte(BME680_H2_MSB_REG) << 4 | ((ReadRegisterOneUByte(BME680_H2_LSB_REG)) >> 4)
    calH3 = ReadRegisterOneSByte(BME680_H3_REG)
    calH4 = ReadRegisterOneSByte(BME680_H4_REG)
    calH5 = ReadRegisterOneSByte(BME680_H5_REG)
    calH6 = ReadRegisterOneUByte(BME680_H6_REG)
    calH7 = ReadRegisterOneSByte(BME680_H7_REG)

    #Gas calibration.
    calGH1 = ReadRegisterOneSByte(BME680_GH1_REG)
    calGH2 = Read2BytesAsShortLSBFirst(BME680_GH2_LSB_REG)
    calGH3 = ReadRegisterOneSByte(BME680_GH3_REG)

    #Heat calibration.
    calResHeatRange = (ReadRegisterOneUByte(BME680_RES_HEAT_RANGE) & 0x30) / 16
    calResHeatVal = ReadRegisterOneSByte(BME680_RES_HEAT_VAL)
    calRangeSwErr = (ReadRegisterOneSByte(BME680_RANGE_SW_ERR) & 0xF0) / 16
...
def ForceRead(gasMeasurementEnabled):
    """Triggers all measurements, and then waits for measurement completion.
    :param gasMeasurementEnabled: Enable or disable gas measurement.
    """
    SetGasMeasurement(gasMeasurementEnabled)

    temp = ReadRegisterOneUByte(BME680_CTRL_MEAS)
...
def ReadTemperature():
    ...
    tempADC = (ReadRegisterOneUByte(BME680_TEMP_MSB) << 12) | (ReadRegisterOneUByte(BME680_TEMP_LSB) << 4) | (ReadRegisterOneUByte(BME680_TEMP_XLSB) >> 4)

    return float(CompensateTemperature(tempADC))

def ReadHumidity():
    """Reads the relative humidity."""
    ForceRead(False)

    humADC = (ReadRegisterOneUByte(BME680_HUM_MSB) << 8) | (ReadRegisterOneUByte(BME680_HUM_LSB))

    return float(CompensateHumidity(humADC))

def ReadPressure():
    """Reads the pressure in Pa."""
    ForceRead(False)

    presADC = (ReadRegisterOneUByte(BME680_PRESS_MSB) << 12) | (ReadRegisterOneUByte(BME680_PRESS_LSB) << 4) | (ReadRegisterOneUByte(BME680_PRESS_XLSB) >> 4)

    return float(CompensatePressure(presADC))
...
def ReadGasResistance():
    ...
    tempADC = (ReadRegisterOneUByte(BME680_TEMP_MSB) << 12) | (ReadRegisterOneUByte(BME680_TEMP_LSB) << 4) | (ReadRegisterOneUByte(BME680_TEMP_XLSB) >> 4)
    gasResADC = (ReadRegisterOneUByte(BME680_GAS_R_MSB) << 2) | (ReadRegisterOneUByte(BME680_GAS_R_LSB) >> 6)
    gasRange = ReadRegisterOneUByte(BME680_GAS_R_LSB) & 0x0F

    calAmbTemp = CompensateTemperature(tempADC)
...
def ReadTPH():
    ...
    resultsTPH[0] = float(CompensateTemperature(tempADC))
    resultsTPH[1] = float(CompensatePressure(presADC))
    resultsTPH[2] = float(CompensateHumidity(humADC))

    return resultsTPH

#Internal Methods

def SetGasMeasurement(state):
    """Turns gas measurement on of off.
    :param state: Gas measurement mode. True for on, false for off.
    """
    configValue = ReadRegisterOneUByte(BME680_CTRL_GAS_1)
...
def SelectHeaterProfileSetPoint(heaterProfileSetPoint):
    ...
    """
    WriteRegister(BME680_CTRL_GAS_1, heaterProfileSetPoint)

def GetMeasuringState():
    """Gets the measuring status from the measuring bit."""
    readValue = ReadRegisterOneUByte(BME680_EAS_STATUS_0)

    return True if ((readValue & 0b00100000) == 0b00100000) else False

def GetGasMeasuringStatus():
...
def GetNewDataStatus():
    """Gets the new data status from the new_data_0 bit."""
    readValue = ReadRegisterOneUByte(BME680_EAS_STATUS_0)

    return True if ((readValue & 0b10000000) == 0b10000000) else False

def CalculateGasResistance(gasResADC, gasRange):
    """Calculates the gas resistance value.
    :param gasResADC: ADC resistance value.
    :param gasRange: ADC range.
    """
    var1 = (1340.0 + 5.0 * calRangeSwErr) * const_array1[gasRange]
    gasres = var1 * const_array2[gasRange] / (gasResADC - 512.0 + var1)

    return gasres

def CalculateHeaterResistance(targetTemp):
